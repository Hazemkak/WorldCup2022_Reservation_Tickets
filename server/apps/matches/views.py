...
class MatchList(APIView):
    ...
    def get(self, request):
        try:
            ...
            return JsonResponse(data={"detail": "Error while retrieving matches"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class MatchDetails(APIView):
    ...
    def get(self, request, id):
        try:
            ...
            if match is None:
                return JsonResponse(data={"detail": "Match not found"}, status=status.HTTP_404_NOT_FOUND)

            serializer = MatchSerializer(match)
            
            return JsonResponse(data={'match': serializer.data}, status=status.HTTP_200_OK)
        except:
            return JsonResponse(data={"detail": "Error while retrieving match"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class CreateMatch(APIView):
    ...
    def post(self, request):
        ...
        if stadium is None:
            return JsonResponse(data={"detail": "Stadium not found"}, status=status.HTTP_404_NOT_FOUND)

        team1 = Team.objects.filter(id=data['team1Id']).first()

        if team1 is None:
            return JsonResponse(data={"detail": "Team 1 not found"}, status=status.HTTP_404_NOT_FOUND)

        team2 = Team.objects.filter(id=data['team2Id']).first()

        if team2 is None:
            return JsonResponse(data={"detail": "Team 2 not found"}, status=status.HTTP_404_NOT_FOUND)

        if team1 == team2:
            return JsonResponse(data={"detail": "Teams must be different"}, status=status.HTTP_400_BAD_REQUEST)

        mainReferee = Referee.objects.filter(id=data['mainRefereeId'], role="1").first()

        if mainReferee is None:
            return JsonResponse(data={"detail": "Main referee not found"}, status=status.HTTP_404_NOT_FOUND)
        
        lineReferee1 = Referee.objects.filter(id=data['lineReferee1Id'], role="0").first()

        if lineReferee1 is None:
            return JsonResponse(data={"detail": "Line referee 1 not found"}, status=status.HTTP_404_NOT_FOUND)
        
        lineReferee2 = Referee.objects.filter(id=data['lineReferee2Id'], role="0").first()

        if lineReferee2 is None:
            return JsonResponse(data={"detail": "Line referee 2 not found"}, status=status.HTTP_404_NOT_FOUND)

        if mainReferee == lineReferee1 or mainReferee == lineReferee2 or lineReferee1 == lineReferee2:
            return JsonResponse(data={"detail": "Referees must be different"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        for match in same_day_matches:
            if match['stadium']['id'] == data['stadiumId']:
                return JsonResponse(data={"detail": "Stadium is already booked for this date"}, status=status.HTTP_400_BAD_REQUEST)

            match_teams_ids = [team['id'] for team in match['teams']]
            
            if len(set(match_teams_ids).intersection([team1.id, team2.id])) > 0:
                return JsonResponse(data={"detail": "Teams must not play on the same day"}, status=status.HTTP_400_BAD_REQUEST)
            ...
            if len(set(match_referees_ids).intersection([mainReferee.id, lineReferee1.id, lineReferee2.id])) > 0:
                return JsonResponse(data={"detail": "Referees must not referee on the same day"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            return JsonResponse(data={"detail": "Error while creating match"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class EditMatch(APIView):
    ...
    def put(self, request, id):
        ...
        # except:
        #     return JsonResponse(data={"detail": "Error while updating match"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)